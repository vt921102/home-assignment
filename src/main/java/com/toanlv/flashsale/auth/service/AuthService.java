...
public class AuthService {
     ...
     *
     * Flow:
     *   1. Detect + normalize identifier
     *   2. Hash password
     *   3. If identifier exists and verified → fail (generic message)
     *   4. If identifier exists and unverified → update password, re-issue OTP
    ...
    public void register(RegisterRequest request) {
        ...
        var normalized = identifierDetector.normalize(
        ...
        if (existing.isPresent()) {
            ...
            otpService.issueOtp(
                    user.getId(), OtpPurpose.REGISTRATION, type, normalized);
            return;
        }
     ...
     * Issues tokens on success — user does not need to login separately.
     *
    ...
    public LoginResponse verifyOtp(VerifyOtpRequest request) {
        ...
        otpService.verifyOtp(
                user.getId(), OtpPurpose.REGISTRATION, request.otp());
     ...
     *
     * Security notes:
     *   - Uses constant-time BCrypt compare for all paths
     *   - Performs fake BCrypt compare when user not found to
    ...
    public LoginResponse login(LoginRequest request) {
        ...
        if (userOpt.isEmpty()) {
            ...
            passwordEncoder.matches(
                    request.password(), "$2a$12$fakehashfortimingnormalization");
    ...
    /**
     * Re-issue OTP for an unverified user.
    ...
    public void resendOtp(String rawIdentifier) {
        ...
        userRepository.findByIdentifierAndIdentifierType(normalized, type)
                .ifPresent(user -> {
                    if (!user.isVerified()) {
                        otpService.issueOtp(user.getId(),
                                OtpPurpose.REGISTRATION, type, normalized);
                    }
                });
    }
    ...
    private LoginResponse issueTokens(User user) {
        var accessToken  = jwtService.issueAccessToken(
                user.getId(), user.getIdentifier(), DEFAULT_ROLE);
