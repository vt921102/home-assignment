...
public class OtpService implements IOtpService {
  ...
  private final IOtpGenerator generator;
  private final RateLimitService rateLimitService;
  private final ApplicationProperties properties;
  private final Clock clock;

  /**
   * Issue a new OTP for the user. Invalidates previous active OTPs for the same user+purpose.
  ...
  public void issueOtp(UUID userId, OtpPurpose purpose, IdentifierType channel, String identifier) {
    ...
    if (!rateLimitService.tryAcquire(rateLimitKey, cfg.resendLimit(), cfg.resendWindow())) {
    ...
    var codeHash = HashUtils.hashOtp(rawOtp, userId.toString(), cfg.pepper());
    var expiresAt = Instant.now(clock).plus(cfg.ttl());

    // Save hashed OTP
    repository.save(OtpVerification.create(userId, codeHash, purpose, expiresAt));
  ...
  /**
   * Verify OTP submitted by user. Uses constant-time comparison to prevent timing attacks.
   *
  ...
  public void verifyOtp(UUID userId, OtpPurpose purpose, String inputOtp) {
    ...
    var inputHash = HashUtils.hashOtp(inputOtp, userId.toString(), cfg.pepper());
