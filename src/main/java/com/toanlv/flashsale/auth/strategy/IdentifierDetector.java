...
public final class IdentifierDetector {
    ...
    public IdentifierType detect(String input) {
        return switch (input) {
            ...
            case String s when s.isBlank() -> throw new BusinessException(
                    ErrorCode.IDENTIFIER_REQUIRED);
            case String s when EMAIL.matcher(s.trim()).matches() ->
                    IdentifierType.EMAIL;
            case String s when PHONE.matcher(s.trim()).matches() ->
     ...
     * Phone: normalize to 0xxxxxxxxx format
     *
