...
public enum ErrorCode {
    ...
    IDENTIFIER_INVALID(1002, "Identifier must be a valid email or Vietnamese phone number", HttpStatus.BAD_REQUEST),
    ...
    OTP_RESEND_LIMIT_EXCEEDED(1105, "OTP resend limit exceeded. Please try again later", HttpStatus.TOO_MANY_REQUESTS),
    ...
    REFRESH_TOKEN_REUSE_DETECTED(1205, "Token reuse detected. All sessions have been invalidated", HttpStatus.UNAUTHORIZED),
    ...
    DAILY_LIMIT_EXCEEDED(3005, "You have already purchased a flash sale item today", HttpStatus.CONFLICT),
    INSUFFICIENT_BALANCE(3006, "Insufficient balance to complete this purchase", HttpStatus.PAYMENT_REQUIRED),
    SESSION_TIME_CONFLICT(3007, "Session time overlaps with an existing session", HttpStatus.CONFLICT),
    ...
    ORDER_CANNOT_BE_CANCELLED(4004, "Order cannot be cancelled in its current state", HttpStatus.BAD_REQUEST),
    ...
    INVENTORY_INCONSISTENCY(5003, "Inventory inconsistency detected", HttpStatus.INTERNAL_SERVER_ERROR),
