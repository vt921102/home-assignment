 ...
 *
 * <p>Used by scheduled workers (OutboxDispatchWorker, ReconciliationWorker) to ensure only one
 * instance processes a batch at a time in a multi-instance deployment.
 *
 * <p>Redisson RLock provides: - Automatic lease renewal (watchdog) while lock is held - Safe
 * release — only the thread that acquired the lock can release it - Java 21 virtual thread
 * compatible (Redisson 3.27+)
 */
@Component
public class LeaderLock {
   ...
   * @param leaseSeconds how long to hold the lock before auto-expiry (Redisson watchdog extends
   *     this automatically while the thread is alive)
  ...
  /**
   * Release the lock. No-op if the current thread does not hold the lock — safe to call
  ...
  /**
   * Execute a task under the leader lock. Acquires the lock, runs the task, then releases in
   * finally. Returns true if the task was executed, false if lock not acquired.
   *
  ...
  /** Check whether the current thread holds the lock. Useful for assertions in tests. */
