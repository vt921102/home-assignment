...
public interface OutboxEventRepository extends JpaRepository<OutboxEvent, UUID> {
  ...
  /**
   * Fetch a batch of pending events that are ready to be processed, skipping any rows locked by
   * another worker instance.
   *
   * <p>FOR UPDATE SKIP LOCKED ensures: - Only one worker processes each row - Workers do not block
   * each other in multi-worker scenarios - Compatible with the LeaderLock pattern (single worker)
   * and future multi-worker scale-out (skip locked handles distribution)
   *
   * <p>next_retry_at <= NOW() filters out events scheduled for future retry.
   */
  @Query(
      value =
          """
