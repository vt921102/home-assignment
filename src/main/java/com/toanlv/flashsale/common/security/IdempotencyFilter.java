...
public class IdempotencyFilter extends OncePerRequestFilter {
  ...
  private static final String PURCHASE_PATH = "/api/v1/flash-sale/purchase";
  ...
  protected void doFilterInternal(
    ...
    if (!isWithinRateLimit(rateLimitKey, cfg.limit(), cfg.window().toSeconds())) {
  ...
  private boolean isWithinRateLimit(String key, int limit, long windowSeconds) {
  ...
  private void writeInvalidKeyResponse(HttpServletResponse response) throws IOException {
    writeJson(
        response,
        HttpStatus.BAD_REQUEST.value(),
        "Bad Request",
  ...
  private void writeConcurrentRequestResponse(HttpServletResponse response) throws IOException {
    writeJson(
        response,
