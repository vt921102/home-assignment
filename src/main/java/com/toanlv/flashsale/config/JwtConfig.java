...
public class JwtConfig {
  ...
  public KeyStore keyStore() {
    ...
    try (InputStream in = Files.newInputStream(Path.of(jwt.keystorePath()))) {
      ...
      throw new IllegalStateException("Failed to load JWT keystore from: " + jwt.keystorePath(), e);
    }
  }
  ...
  public PrivateKey jwtPrivateKey(KeyStore keyStore) {
    var jwt = properties.jwt();
    try {
      var key = keyStore.getKey(jwt.keyAlias(), jwt.keystorePassword().toCharArray());
      if (!(key instanceof PrivateKey pk)) {
        throw new IllegalStateException("Alias '" + jwt.keyAlias() + "' is not a PrivateKey");
      }
      return pk;
    } catch (Exception e) {
  ...
  public PublicKey jwtPublicKey(KeyStore keyStore) {
    ...
    try {
      ...
      if (cert == null) {
        throw new IllegalStateException("No certificate found for alias: " + jwt.keyAlias());
      }
  ...
  public JwtParser jwtParser(PublicKey publicKey) {
    return Jwts.parser().verifyWith(publicKey).requireIssuer(properties.jwt().issuer()).build();
  }
}
