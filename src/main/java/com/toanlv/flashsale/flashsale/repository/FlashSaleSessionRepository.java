...
@Repository
public interface FlashSaleSessionRepository extends JpaRepository<FlashSaleSession, UUID> {

  /**
   * Find all active sessions for a given date and time window. Used by FlashSaleQueryService to
  ...
  List<FlashSaleSession> findBySaleDateOrderByStartTimeAsc(LocalDate saleDate);
}
