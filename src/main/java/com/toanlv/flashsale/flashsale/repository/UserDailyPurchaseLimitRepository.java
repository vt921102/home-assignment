...
public interface UserDailyPurchaseLimitRepository
   ...
   *
   * <p>Enforces the "1 user = 1 purchase per day" rule atomically. UNIQUE constraint on (user_id,
   * purchase_date) is the DB guard.
   *
   * <p>Returns 1 if inserted (first purchase today), 0 if already exists. Caller throws
   * DAILY_LIMIT_EXCEEDED if 0 is returned.
   *
   * <p>This is safe under concurrent requests — PostgreSQL serializes at the constraint level, not
  ...
  @Query(
      value =
          """
            ...
            """,
      nativeQuery = true)
  int insertIfAbsent(@Param("userId") UUID userId, @Param("date") LocalDate date);
}
