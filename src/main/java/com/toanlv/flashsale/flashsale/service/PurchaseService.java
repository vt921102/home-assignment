...
public class PurchaseService {
    ...
    public PurchaseService(
        ...
        this.rules = List.of(timeWindowRule, stockRule,
                        balanceRule, dailyLimitRule)
                .stream()
                .sorted(Comparator.comparingInt(
                        PurchaseEligibilityRule::order))
                .toList();
     ...
     *
     * Pre-transaction:
     *   1. Idempotency cache lookup — return cached response on hit
     *
     * Transactional (with retry on optimistic lock conflict):
     *   2. Load session item with PESSIMISTIC_READ lock
     *   3. Load user balance
     *   4. Run eligibility rules (TimeWindow, Stock, Balance, DailyLimit)
     *   5. Atomic stock decrement (optimistic lock)
     *   6. Atomic balance deduction (WHERE balance >= price)
     *   7. Create order + balance transaction
     *   8. Publish FLASH_SALE_PURCHASED outbox event
     *
     * Post-commit:
     *   9. Cache response in Redis (afterCommit hook)
     */
    public PurchaseResponse purchase(
        ...
        return retryTemplate.execute(
                ctx -> doPurchase(userId, sessionItemId, idempotencyKey));
    }
    ...
    public PurchaseResponse doPurchase(
            UUID userId,
            UUID sessionItemId,
            String idempotencyKey) {

        // Load item with shared lock
        var item = itemRepository.findByIdForPurchase(sessionItemId)
                .orElseThrow(() -> new BusinessException(
                        ErrorCode.SESSION_ITEM_NOT_FOUND));

        // Load user balance for rule evaluation
        var balance = userRepository.findBalanceById(userId)
                .orElseThrow(() -> new BusinessException(
        ...
        var stockUpdated = itemRepository.decrementSold(
                item.getId(), item.getVersion());
        if (stockUpdated == 0) {
            // Version mismatch or sold out — trigger retry
            throw new OptimisticLockingFailureException(
                    "Stock contention on item " + item.getId());
        }

        // Atomic balance deduction — WHERE balance >= price
        var balanceUpdated = userRepository.deductBalance(
                userId, item.getSalePrice());
        if (balanceUpdated == 0) {
            throw new BusinessException(ErrorCode.INSUFFICIENT_BALANCE);
        }

        // Persist order (via OrderService — same transaction)
        var newBalance = userRepository.findBalanceById(userId)
                .orElse(BigDecimal.ZERO);

        var order = orderService.createFlashSaleOrder(
                userId,
                idempotencyKey,
