 ...
 *
 * Sealed to make the set of rules explicit and discoverable.
 * New rules must be added to the permits clause — intentional
 * design decision to keep rule inventory visible at a glance.
 *
 * Rules are ordered via order() and applied sequentially by PurchaseService.
 * First rule that throws stops evaluation (fail-fast).
 */
