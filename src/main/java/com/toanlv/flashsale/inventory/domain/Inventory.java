...
public class Inventory {
    ...
    public void restock(int quantity) {
        if (quantity <= 0) {
            throw new IllegalArgumentException(
                    "Restock quantity must be positive, got: " + quantity);
        }
     ...
     * Called on order refund or cancellation.
     *
    ...
    /**
     * Confirm sale — remove from reserved and total.
     * Called after order is fully completed.
     *
    ...
    public void confirmSale(int quantity) {
        if (quantity <= 0) {
            throw new IllegalArgumentException(
                    "Confirm quantity must be positive, got: " + quantity);
        }
        if (this.reservedQuantity < quantity) {
            throw new IllegalStateException(
    ...
    /**
     * Verify the inventory invariant: total = available + reserved.
     * Used in reconciliation and tests.
     */
