package com.toanlv.flashsale.inventory.service.impl;
...
public class InventorySyncService implements IInventorySyncService {
   ...
   *
   * <p>Idempotency is enforced at two levels: 1. audit log UNIQUE on source_event_id — duplicate
   * event throws DataIntegrityViolationException, caught and skipped. 2. UPDATE WHERE available >=
  ...
  public void handlePurchase(OutboxEvent event) {
    ...
    if (!tryInsertAuditLog(productId, event.getId(), -quantity, "FLASH_SALE_PURCHASE")) {
      log.info("Duplicate purchase event skipped: eventId={}", event.getId());
      return;
    }

    var updated = inventoryRepository.decrementAvailable(productId, quantity);

    if (updated == 0) {
      throw new BusinessException(
          ErrorCode.INVENTORY_INSUFFICIENT,
          "Cannot decrement inventory for productId=" + productId + ", quantity=" + quantity);
    }

    log.debug("Inventory decremented: productId={}, qty={}", productId, quantity);
  }
  ...
  public void handleRestock(OutboxEvent event) {
    ...
    if (!tryInsertAuditLog(productId, event.getId(), quantity, "PRODUCT_RESTOCKED")) {
      log.info("Duplicate restock event skipped: eventId={}", event.getId());
      return;
    }

    var updated = inventoryRepository.incrementAvailable(productId, quantity);

    if (updated == 0) {
      throw new BusinessException(
          ErrorCode.INVENTORY_NOT_FOUND, "Inventory not found for productId=" + productId);
    }

    log.debug("Inventory restocked: productId={}, qty={}", productId, quantity);
  }
  ...
  public void handleRefund(OutboxEvent event) {
    ...
    if (!tryInsertAuditLog(productId, event.getId(), quantity, "ORDER_REFUND")) {
      log.info("Duplicate refund event skipped: eventId={}", event.getId());
      return;
    }

    var updated = inventoryRepository.releaseReserved(productId, quantity);
    ...
    log.debug("Inventory released: productId={}, qty={}", productId, quantity);
  }
  ...
  /**
   * Admin-triggered direct restock. Publishes an outbox event so inventory sync is consistent with
   * purchase-triggered sync.
   */
  @Override
  @Transactional
   ...
   */
  private boolean tryInsertAuditLog(UUID productId, UUID sourceEventId, int delta, String reason) {
    try {
      auditLogRepository.save(InventoryAuditLog.create(productId, sourceEventId, delta, reason));
      return true;
  ...
  private UUID extractUUID(OutboxEvent event, String key) {
    ...
    if (value == null) {
      throw new IllegalArgumentException(
          "Missing payload key '" + key + "' in event " + event.getId());
    }
