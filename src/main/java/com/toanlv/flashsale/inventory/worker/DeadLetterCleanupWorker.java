 ...
 *
 * Dead letter events are kept for 7 days for manual inspection,
 * then removed to prevent unbounded table growth.
 *
 * Runs at 02:00 AM daily to avoid peak hours.
 */
@Component
public class DeadLetterCleanupWorker {

    private static final Logger log =
            LoggerFactory.getLogger(DeadLetterCleanupWorker.class);
    ...
    protected void doCleanup() {
        var cutoff = java.time.Instant.now()
                .minus(RETENTION_DAYS, java.time.temporal.ChronoUnit.DAYS);

        var batch = outboxEventRepository
                .fetchDeadLetterBatch(500);

        var toDelete = batch.stream()
                .filter(e -> e.getCreatedAt().isBefore(cutoff))
                .toList();

        if (toDelete.isEmpty()) {
            log.info("Dead letter cleanup: no events older than {} days",
                    RETENTION_DAYS);
            return;
        }
        ...
        log.info("Dead letter cleanup: deleted {} events older than {} days",
                toDelete.size(), RETENTION_DAYS);
    }
}
