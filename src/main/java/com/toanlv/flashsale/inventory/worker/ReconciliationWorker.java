 ...
 *
 * <p>Scans for inventory rows where the invariant total = available + reserved is violated.
 *
 * <p>In a correctly functioning system, this should never fire. If it does, it logs an alert and
 * increments a metric for monitoring systems (Prometheus / Grafana) to catch.
 *
 * <p>Does NOT auto-correct — corrections require human review to avoid masking bugs silently.
 */
@Component
public class ReconciliationWorker {
  ...
  public ReconciliationWorker(
      InventoryRepository inventoryRepository, LeaderLock leaderLock, MeterRegistry meterRegistry) {
  ...
  public void reconcile() {
    leaderLock.runIfLeader(LOCK_NAME, LOCK_LEASE_SEC, this::doReconcile);
  }
  ...
  protected void doReconcile() {
    ...
    for (var inv : mismatches) {
      ...
      meterRegistry
          .counter("inventory.reconciliation.mismatch", "productId", inv.getProductId().toString())
          .increment();
    }

    log.warn(
        "Inventory reconciliation found {} inconsistency(ies). " + "Manual review required.",
        mismatches.size());
  }
}
