...
public class OrderController {
    ...
    public ResponseEntity<Page<OrderSummaryDto>> getMyOrders(
        ...
        return ResponseEntity.ok(
                orderService.findByUser(user.userId(), pageable));
    }
    ...
    public ResponseEntity<OrderDto> getMyOrder(
        ...
        return ResponseEntity.ok(
                orderService.findByIdAndUser(orderId, user.userId()));
    }

    @Operation(
            summary = "Get my balance transactions",
            description = "Paginated balance transaction history "
    ...
    public ResponseEntity<Page<BalanceTransactionDto>> getMyTransactions(
            ...
            @PageableDefault(size = 20, sort = "createdAt") Pageable pageable) {
        return ResponseEntity.ok(
                orderService.findTransactionsByUser(user.userId(), pageable));
    }
}