...
public interface OrderRepository extends JpaRepository<Order, UUID> {
    ...
    /**
     * Find orders for a user with items eagerly loaded.
     * Avoids N+1 when rendering order list with item counts.
     */
    @Query("""
     ...
     * Used when handling duplicate purchase requests.
     */
    Optional<Order> findByIdempotencyKey(String idempotencyKey);

    /**
     * Check if a user has a completed order for a specific
     * flash sale session item (source_ref_id).
