package com.toanlv.flashsale.order.service.impl;
...
public class OrderService implements IOrderService {
  ...
  public Page<OrderSummaryDto> findByUser(UUID userId, Pageable pageable) {
    return orderRepository.findByUserId(userId, pageable).map(OrderSummaryDto::from);
  }
  ...
  @Override
  @Transactional(readOnly = true)
  public Page<BalanceTransactionDto> findTransactionsByUser(UUID userId, Pageable pageable) {
  ...
  /**
   * Persist a completed flash sale order together with its balance transaction record.
   *
   * <p>Called within PurchaseService's @Transactional boundary — both writes are atomic with the
   * stock decrement and balance deduction.
   *
  ...
  public Order createFlashSaleOrder(
    ...
    var order = Order.createFlashSaleOrder(userId, idempotencyKey, unitPrice);

    var item = OrderItem.of(productId, productName, 1, unitPrice, sessionItemId);
    ...
    balanceTxRepository.save(
        BalanceTransaction.debit(
            userId, saved.getId(), unitPrice, newBalance, "FLASH_SALE_PURCHASE"));

    return saved;
  }

  /** Find order by idempotency key. Returns empty if not found — caller decides what to do. */
  @Override
  @Transactional(readOnly = true)
