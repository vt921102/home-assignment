...
public class ProductService {
    ...
    /**
     * Paginated product list with optional category and search filters.
     * Only returns ACTIVE products for public access.
     */
    @Cacheable(
            value = "product-catalog",
            key = "#categoryId + ':' + #search + ':' + #pageable.pageNumber"
                    + "+ ':' + #pageable.pageSize"
    )
    @Transactional(readOnly = true)
    public Page<ProductDto> findActive(
            UUID categoryId,
            String search,
            Pageable pageable) {
     ...
     * Automatically creates a corresponding inventory record with 0 stock.
     */
     ...
     * Soft delete via DISCONTINUED — preserves order history references.
     */
    @CacheEvict(value = "product-catalog", allEntries = true)
    @Transactional
