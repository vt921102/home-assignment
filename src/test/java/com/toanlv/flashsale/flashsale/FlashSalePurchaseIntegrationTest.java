...
class FlashSalePurchaseIntegrationTest {
    ...
    void setUp() {
        ...
        var category = categoryRepository.save(
                ProductCategory.create("Electronics", null));
        var product = productRepository.save(
                Product.create("TEST-SKU-001", "Test Product",
                        "Description", BigDecimal.valueOf(100_000),
                        null, category));
        inventoryRepository.save(Inventory.initWithStock(
                product.getId(), 100));
        ...
        sessionItem = itemRepository.save(
                FlashSaleSessionItem.create(
                        session, product,
                        BigDecimal.valueOf(99_000), 10, 1));
    }
    ...
    void concurrentPurchases_neverExceedStock() throws InterruptedException {
        ...
        var updatedItem = itemRepository.findById(sessionItem.getId())
                .orElseThrow();
    ...
    @Test
    void purchase_withSameIdempotencyKey_createsOnlyOneOrder()
    ...
    private List<User> createUsers(int count, BigDecimal balance) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            userRepository.deductBalance(saved.getId(),
                    BigDecimal.ZERO.subtract(balance));
